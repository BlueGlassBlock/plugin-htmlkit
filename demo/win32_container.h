...
class win32_container : public litehtml::document_container {
  ...
  public:
    ...
    litehtml::string
    resolve_color(const litehtml::string &color) const override;
  ...
  protected:
    ...
    void make_url_utf8(const char *url, const char *basepath,
                       std::wstring &out);
    virtual void get_client_rect(litehtml::position &client) const = 0;

    // get_image is called by load_image.
    // if url_or_path is URL then get_image may return 0, the image should be
    // added later by add_image when it becomes available
    ...
    virtual void draw_img_bg(HDC hdc, uint_ptr img,
                             const litehtml::background_layer &layer) = 0;

    virtual void draw_ellipse(HDC hdc, int x, int y, int width, int height,
                              litehtml::web_color color, int line_width) = 0;
    virtual void fill_ellipse(HDC hdc, int x, int y, int width, int height,
                              litehtml::web_color color) = 0;
    virtual void fill_rect(HDC hdc, int x, int y, int width, int height,
                           litehtml::web_color color) = 0;

  private:
    static int CALLBACK EnumFontsProc(const LOGFONT *lplf,
                                      const TEXTMETRIC *lptm, DWORD dwType,
                                      LPARAM lpData);
