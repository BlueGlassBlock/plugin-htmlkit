...
windows_container::windows_container(void) {
    ...
    CoCreateInstance(CLSID_CMultiLanguage, NULL, CLSCTX_ALL,
                     IID_IMLangFontLink2, (void **)&m_font_link);
}
...
litehtml::uint_ptr
windows_container::create_font(const litehtml::font_description &descr,
    ...
    cairo_font *fnt = new cairo_font(
        m_font_link, fnt_name.c_str(), descr.size, descr.weight,
    ...
    if (fm) {
        ...
        if (descr.style || descr.decoration_line) {
            fm->draw_spaces = true;
        } else {
...
void windows_container::draw_text(litehtml::uint_ptr hdc, const char *text,
...
litehtml::string
windows_container::resolve_color(const litehtml::string &color) const {
    struct custom_color {
        const char *name;
        int color_index;
    };
