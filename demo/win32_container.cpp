...
wstring utf8_to_utf16(string str)
{
	...
	if (len > 0)
	{
		wstr.resize(len - 1); // Exclude null terminator
		MultiByteToWideChar(CP_UTF8, 0, str.c_str(), -1, &wstr[0], len);
	}
	return wstr;
}

string utf16_to_utf8(wstring wstr)
{
	string str;
	int len = WideCharToMultiByte(CP_UTF8, 0, wstr.c_str(), -1, NULL, 0, NULL, NULL);
	if (len > 0)
	{
		str.resize(len - 1); // Exclude null terminator
...
litehtml::uint_ptr	win32_container::create_font(const font_description& descr, const document* doc, litehtml::font_metrics* fm)
{
	wstring font_name;
	string_vector fonts;
	...
	if (fm)
	{
		HDC hdc = m_tmp_hdc ? m_tmp_hdc : GetDC(NULL);
		if (hdc)
		{
			...
			fm->draw_spaces = descr.style || descr.decoration_line;
