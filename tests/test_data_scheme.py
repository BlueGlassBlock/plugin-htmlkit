...
data_scheme = Path(__file__).parent / "data_scheme"

HTML_SOURCES = [name for name in data_scheme.iterdir() if name.suffix == ".html"]
...
@pytest.mark.parametrize("html_file", HTML_SOURCES)
async def test_render_image_from_data_scheme(
    html_file, image_format, regen_ref, output_img_dir, native
):
    ...
    filename = (
        f"data_scheme_{html_file.stem}{'_native' if native else ''}.{image_format}"
    )
